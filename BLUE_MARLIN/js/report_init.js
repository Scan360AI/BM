    ...
    // --- PARTE 2: Economico ---
         ...
         try {
             ...
             const options = {
                ...
                scales: {
                     ...
                     y1: { position: 'right', title: { display: true, text: 'EBITDA %' }, grid: { drawOnChartArea: false }, suggestedMin: 0, suggestedMax: 20, ticks: { callback: function(v) { return v + '%';}} }
                 },
     ...
     if (document.getElementById('marginalityChart')) {
         try {
             ...
             const options = {
                 ...commonChartOptions,
                 scales: { y: { ...commonChartOptions.scales.y, title: { display: true, text: 'Percentuale sui Ricavi (%)' }, suggestedMax: 50, ticks: { callback: function(v) { return v + '%';}} } },
     ...
     if (document.getElementById('profitabilityIndicesChart')) { // Rinominato da profitabilityChart
         try {
            const data = getProfitabilityIndicesData(); // Assicurati che la funzione esista in config
             const options = {
                 ...commonChartOptions,
                 scales: { y: { ...commonChartOptions.scales.y, title: { display: true, text: 'Percentuale (%)' }, suggestedMin: -10, suggestedMax: 30, ticks: { callback: function(v) { return v + '%';}} } },
    ...
    if (document.getElementById('workingCapitalCycleChart')) {
         try {
            const data = getWorkingCapitalCycleData();
             const options = {
                 ...commonChartOptions,
                 scales: { y: { ...commonChartOptions.scales.y, title: { display: true, text: 'Giorni' }, suggestedMax: 300 } },
     ...
     if (document.getElementById('zscoreChart')) {
         try {
            const data = getZscoreData();
             const options = {
                 ...commonChartOptions,
                 scales: { y: { ...commonChartOptions.scales.y, title: { display: true, text: 'Z-Score' }, suggestedMin: 0, suggestedMax: 3 } },
