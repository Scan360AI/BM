...
function handleLogin(event) {
    ...
    if ((username === 'AdminBlueMarlin' && password === 'BlueMarlin2025') || (username && password)) {
...
function checkLoginStatus() {
    ...
    try {
        ...
        // Verifica scadenza sessione (8 ore)
        const loginTime = localStorage.getItem('scanLoginTime');
        if (loggedIn && loginTime) {
            const loginDate = new Date(loginTime);
            const currentDate = new Date();
            const hoursElapsed = (currentDate - loginDate) / (1000 * 60 * 60);
            
            if (hoursElapsed > 8) {
                console.warn("Sessione scaduta dopo 8 ore.");
                localStorage.removeItem('scanUserLoggedIn');
                localStorage.removeItem('scanUsername');
                localStorage.removeItem('scanLoginTime');
                loggedIn = false;
